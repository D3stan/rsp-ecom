...
// Social media icons from Simple Icons (copied from home.tsx)
...
export default function PublicLayout({ children, transparent = false, currentPage }: PublicLayoutProps) {
    const { t } = useTranslation();

    return (
        <div className="min-h-screen bg-white">
            {/* Header */}
            <Header currentPage={currentPage} transparent={transparent} />

            {/* Main Content */}
            <main className={transparent ? '' : 'pt-0'}>{children}</main>
            ...
            <footer className="bg-black py-12 text-white">
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    <div className="grid grid-cols-1 gap-8 md:grid-cols-4">
                        <div>
                            <h3 className="mb-4 text-xl font-bold">{import.meta.env.VITE_APP_NAME || 'Store'}</h3>
                    ...
                    <div className="mt-8 border-t border-gray-800 pt-8 text-center text-gray-400">
                        <p>
                            &copy; 2025 {import.meta.env.VITE_APP_NAME || 'Store'}. {t('rights_reserved')}
                        </p>
                    </div>
                </div>
            </footer>
